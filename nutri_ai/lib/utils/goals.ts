...
export const getCalorieAdjustmentForRate = (
  ...
  return goalType === WeightGoalType.Lose
    ? -dailyCalorieChange
    : dailyCalorieChange;
};
...
export const getGoalSummary = (user: IUser, goals: NutritionGoals): string => {
  ...
  if (user.targetWeightKg && goals.estimatedTimeToGoal) {
    parts.push(
      `Estimated time to reach ${user.targetWeightKg} kg: ${goals.estimatedTimeToGoal} weeks`,
    );
  }
...
export const validateGoals = (
  ...
  if (user.targetWeightKg) {
    ...
    if (targetBMI < 16) {
      warnings.push(
        "Target weight results in severely underweight BMI. Please consult a healthcare professional.",
      );
    }

    if (targetBMI > 40) {
      warnings.push(
        "Target weight results in very high BMI. Consider setting intermediate goals.",
      );
    }

    if (weightDifference > 30) {
      warnings.push(
        "Large weight change goal detected. Consider breaking this into smaller, achievable milestones.",
      );
    }
  }
  ...
  if (
    user.weightGoalType === WeightGoalType.Lose &&
    user.weightLossRate === WeightLossRate.Aggressive
  ) {
    const bmi = user.weightKg / Math.pow(user.heightCm / 100, 2);
    if (bmi < 25) {
      warnings.push(
        "Aggressive weight loss rate not recommended for individuals with BMI < 25.",
      );
    }
  }

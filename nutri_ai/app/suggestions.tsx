...
The value of lastMeal is: ${lastMeal}

You MUST follow these rules exactly:

- lastMeal = 0:
  - breakfast, lunch, dinner → all required
- lastMeal = 1:
  - lunch, dinner → required
  - breakfast → name must be "none"
- lastMeal = 2:
  - dinner → required
  - breakfast and lunch → name must be "none"

The total number of nutrients per category you suggest should total the remaining amounts. 
...
If some of the meals is not suggested, fill "name" with "none".
...
`;
  ...
  const filteredMeals = Object.keys(mealSuggestions).filter(
    (key) => mealSuggestions[key].name.toLowerCase() !== "none"
  );
  ...
  return (
    ...
    >
      ...
      {filteredMeals.length === 0 && (
        <Text style={[styles.errorText, { color: isDark ? "#fff" : "#111", textAlign: "center" }]}>

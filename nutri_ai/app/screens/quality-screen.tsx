...
export default function QualityScreen() {
  ...
  const { avgQuality, totalMeals, bestMeals, worstMeals, chartData } = useMemo(() => {
    if (meals.length === 0) {
      return { avgQuality: 0, totalMeals: 0, bestMeals: [], worstMeals: [], chartData: [] };
    }

    const totalQ = meals.reduce((sum, m) => sum + m.mealQuality.mealQualityScore, 0);
    const avg = totalQ / meals.length;

    // Top meals by quality
    const bestMeals = [...meals].sort((a, b) => b.mealQuality.mealQualityScore - a.mealQuality.mealQualityScore).slice(0, 3);
    const worstMeals = [...meals].sort((a, b) => a.mealQuality.mealQualityScore - b.mealQuality.mealQualityScore).slice(0, 3);

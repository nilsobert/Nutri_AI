...
\author{Laura Kotalczyk \\ Manuel Mühlberger \\ \textbf{Group 4}}\date{\today}
\maketitle
\vfill
\newpage
%
...
%
\noindent
The vision of this project is to develop a more accurate and user-friendly mobile application for tracking daily nutritional intake by combining visual and spoken input through AI. Our goal is to address the common pain points of current calorie-tracking tools, such as time-consuming manual entry, limited context in meal logging as well as wrongful calorie estimation by existing AI-enabled calorie trackers.
...
The goal is to reduce user effort and improve accuracy through its multi-modal approach, particularly for home-cooked and complex meals that are frequently misclassified or falsely estimated in existing tools. The user can also browse a simple calendar view to display daily food intake and progress towards goals. Insights are provided to the user as well, so he can gain information on metrics such as average meal quality, longest logging streaks and many others.
\\
This solution aims to make nutrition tracking simpler, smarter, and more accessible. It helps users stay aware of their intake and make informed choices, without requiring expertise meticulous manual logging.
...
\begin{minipage}{\textwidth}
    ...
    \begin{tabularx}{\textwidth}{l X}
        ...
        \addlinespace[10pt]
        Test Data \& Results & Uses 10–20 real example meals (e.g., homemade pancakes) with known nutrition values to test accuracy. \\
        \addlinespace[10pt]
...
To keep the project focused and manageable, we defined the following boundaries and mandatory/optional features.
\newline
...
\begin{itemize}
    ...
    \item Integration with fitness trackers / Apple Health to increase available calories
    %\item Integration of a user's training activities 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{typst_plots_out/gantt_chart.pdf}
...
\begin{figure}[htbp]
    ...
    \caption{High-Level System Architecture of NutriAI.\protect\footnotemark}
    \label{fig:system_design}
\end{figure}

\footnotetext{Note: During later development, \texttt{qwen2.5-vl} was replaced with \texttt{qwen3-vl-235b-a22b-instruct}.}
...
\noindent
Since we were aiming at building a mobile app that runs cross-platform, we opted for Expo (version 54), after some research on possible options. Expo is a full-stack React Native framework that offers a rich ecosystem of pre-built components and tools, known for its simple setup, fast prototyping, and smooth development workflow \cite{expo}. Its accessibility made it a strong fit for our team, particularly given the varying levels of experience among team members. Thus, the client can either be an iOS or Android user, although development focused on iOS deployment since the team did not have access to any Android devices and to keep the project manageable. In the current development build, the app requires the user to have a custom certificate installed on their device, for encrypted communication with the NutriAI server. This limitation is due to time constraints, and for a production build it would be possible to host a universally trusted Let's Encrypt certificate on the server \cite{lets}.\\
...
When the user uses the app to sign up, login or log a meal, corresponding requests are sent to the proxy server.
 ...
 Pangolin manages the "allowed" request flow by utilizing Newt, a user-space WireGuard tunnel client, to securely forward traffic to the on-premise server. This distributed architecture shields the internal infrastructure from malicious actors by punching through NAT and firewall restrictions on the main server side without requiring open ports. Furthermore, this setup facilitates high availability and failover, as a single Pangolin instance can coordinate and route traffic across multiple Newt site connectors.
...
\noindent
To track tickets and tasks, we used Trello, a Kanban-style tool that organizes work into boards, lists, and cards and tracks progress from “to-do” to “done” \cite{trello}. For a screenshot taken from our project's board, refer to Figure \ref{fig:trello}.
Apart from using a project management tool, we also created a WhatsApp group with all the team members to agree on meeting dates and discuss immediate questions or provide advice faster.
\\
With respect to roadmap planning, our overall strategy, in terms of application design, was to go from the coarse overview to the more granular subsystems, and, thereby, develop in a step-by-step fashion. We started with a target audience analysis to inform and guide the requirements engineering process, resulting in the functional and non-functional requirements our application needed to meet. Following our requirements collection, we then proceeded with system design. 
Once an initial architecture was established, we began with the implementation, making adjustments as needed to accommodate changes or unforeseen challenges.
A more detailed explanation for our techniques employed during system design as well as database design will be given in sections \ref{chap:sys} and \ref{chap:db}, respectively.
After completing the overall implementation, we initiated user acceptance testing to gather feedback on the application and identify any remaining weaknesses or areas needing improvemen in the future.
...
During development, we applied prompt engineering techniques to evaluate and compare two distinct prompt sets (system and user prompts) for our main feature - one simpler and one more complex.
...
\label{chap:eval}
\noindent
To evaluate the system's accuracy, we benchmarked NutriAI against three competitor apps (arise, Calorix AI, Calorie Cue AI) using the previously established ground truth dataset. We tested our solution in "Photo only" and "Photo + Audio" modes. As shown in Table \ref{tab:mean_percentage_error}, all apps tended to overestimate calories, though NutriAI achieved the best performance in this regard. While audio input notably improved carbohydrate estimation (45.7\% to 38.2\%), it inadvertently increased error in fat estimation (14.1\% to 44.6\%). However, given the limited sample size of the dataset, these trends should be interpreted as indicative rather than definitive.
...
\noindent
Figure \ref{fig:overall_mae_bar} displays the Overall Mean Absolute Percentage Error (MAPE). NutriAI with audio input achieved the lowest overall error rate of approximately 45\%, a clear improvement over the "Photo only" mode which resulted in a MAPE of ~52\%. In contrast, competitor apps demonstrated considerably higher error rates, with "arise" performing worst at over 80\% MAPE.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{typst_plots_out/overall_mae_bar.pdf}
\caption{Overall Mean Absolute Percentage Error (MAPE) by App.\protect\footnotemark}
\label{fig:overall_mae_bar}
\end{figure}
\footnotetext{Overall MAPE is computed as $\mathrm{MAPE}_{\mathrm{overall}} = \tfrac{1}{4}\sum_{m\in\{\mathrm{Cal},\mathrm{Carb},\mathrm{Prot},\mathrm{Fat}\}} \mathrm{MAPE}_m$. Unlike the signed MPE in Table \ref{tab:mean_percentage_error}, it is unsigned (error magnitude), so rankings can differ.}
\noindent
The error distribution is visualized in Figure \ref{fig:percentage_error_boxplots}. The boxplots reveal that NutriAI with audio input maintains the tightest distribution around the zero-error line across most categories. While fat estimation showed slightly increased variance with audio included, the overall consistency is superior to competitors like Calorix AI and arise, which exhibit large interquartile ranges and extreme outliers.
...
It was also used to assist with debugging problems occuring with LaTeX and diagram generation.
